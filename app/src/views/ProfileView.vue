  ...
  <div style="display:flex;height:100vh;background:var(--bg-base);position:relative;overflow:hidden">
    ...
    <div style="flex:1;display:flex;flex-direction:column;overflow:hidden;position:relative;z-index:1">
      <AppHeader :breadcrumbs="[{ label: 'Profilo' }]">
        <template #actions>
          <button @click="auth.logout().then(() => $router.push({ name: 'login' }))" style="padding:6px 14px;border-radius:8px;border:1px solid rgba(var(--rgb-border),0.08);background:transparent;color:rgba(var(--rgb-text),0.5);font-size:0.8rem;cursor:pointer;transition:all 0.2s"
      ...
      <div style="flex:1;overflow-y:auto;padding:28px 32px">
        <div style="max-width:720px;margin:0 auto">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:24px">
            <div v-for="s in statCards" :key="s.label" style="padding:16px;border-radius:14px;background:rgba(var(--rgb-surface),0.7);border:1px solid rgba(var(--rgb-border),0.06);text-align:center">
              <div :style="{ fontSize: '1.3rem', fontWeight: 700, color: s.color, marginBottom: '4px' }">{{ s.value }}</div>
              <div style="font-size:0.72rem;color:rgba(var(--rgb-text),0.38)">{{ s.label }}</div>
            </div>
          </div>

          <!-- XP Progress -->
          <div style="margin-bottom:24px;padding:18px;border-radius:14px;background:rgba(var(--rgb-surface),0.7);border:1px solid rgba(var(--rgb-border),0.06)">
          ...
          <div style="margin-bottom:24px" v-if="stats.badges?.length">
            <h3 style="margin:0 0 14px;font-size:0.75rem;font-weight:600;color:rgba(var(--rgb-text),0.38);letter-spacing:0.08em">BADGE</h3>
            <div style="display:flex;gap:12px;flex-wrap:wrap">
              <div v-for="id in stats.badges" :key="id" style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:12px;background:rgba(var(--rgb-surface),0.7);border:1px solid rgba(var(--rgb-border),0.07)">
                ...
                <div>
                  <div style="font-size:0.82rem;font-weight:600;color:var(--text-1)">{{ badgeData[id]?.name || id }}</div>
                  <div style="font-size:0.7rem;color:rgba(var(--rgb-text),0.38)">{{ badgeData[id]?.desc || '' }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Streak calendar -->
          <div>
            <h3 style="margin:0 0 14px;font-size:0.75rem;font-weight:600;color:rgba(var(--rgb-text),0.38);letter-spacing:0.08em">ATTIVITÀ QUESTA SETTIMANA</h3>
            <div style="display:flex;gap:8px">
              <div v-for="(d, i) in weekDays" :key="i" style="flex:1;display:flex;flex-direction:column;align-items:center;gap:5px">
                <div :style="{
                  ...
                  background: i < stats.streak && i < 7 ? 'rgba(99,102,241,0.25)' : 'rgba(var(--rgb-border),0.04)',
                  border: `1px solid ${i < stats.streak && i < 7 ? 'rgba(99,102,241,0.35)' : 'rgba(var(--rgb-border),0.06)'}`,
                }">
