<template>
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;background:var(--bg-base);position:relative;overflow:hidden">
    <StarBackground />
    <div style="position:relative;z-index:1;width:100%;max-width:400px">
      ...
      <div style="text-align:center;margin-bottom:32px">
        ...
        <h1 style="margin:0 0 4px;font-size:1.6rem;font-weight:700;color:var(--text-1);letter-spacing:-0.02em">Arabel Learn</h1>
        <p style="margin:0;font-size:0.85rem;color:rgba(var(--rgb-text),0.4)">Impara a programmare, un livello alla volta</p>
      </div>

      <!-- Card -->
      <div style="background:rgba(var(--rgb-surface),0.8);backdrop-filter:blur(20px);border:1px solid rgba(var(--rgb-border),0.08);border-radius:20px;padding:28px">
        <!-- Tabs -->
        <div style="display:flex;margin-bottom:24px;background:rgba(var(--rgb-border),0.04);border-radius:10px;padding:3px">
          <button v-for="m in ['login','register']" :key="m" @click="mode = m" :style="{
            ...
            color: mode === m ? '#fff' : 'rgba(var(--rgb-text),0.45)',
        ...
        <!-- Google OAuth -->
        <a :href="`${apiBase}/auth/google`" style="display:flex;align-items:center;justify-content:center;gap:10px;padding:11px;border-radius:10px;border:1px solid rgba(var(--rgb-border),0.1);background:rgba(var(--rgb-border),0.04);color:var(--text-1);font-size:0.9rem;font-weight:500;text-decoration:none;transition:background 0.2s;margin-bottom:4px"
          @mouseover="e => e.currentTarget.style.background='rgba(var(--rgb-border),0.08)'"
          @mouseout="e => e.currentTarget.style.background='rgba(var(--rgb-border),0.04)'">
        ...
        <div style="display:flex;align-items:center;gap:10px;margin:4px 0">
          <div style="flex:1;height:1px;background:rgba(var(--rgb-border),0.07)"/>
          <span style="font-size:0.72rem;color:rgba(var(--rgb-text),0.3)">oppure</span>
          <div style="flex:1;height:1px;background:rgba(var(--rgb-border),0.07)"/>
        </div>
